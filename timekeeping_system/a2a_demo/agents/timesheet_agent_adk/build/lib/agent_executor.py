...
class TimeSheetAgentExecutor(AgentExecutor):
    """An AgentExecutor that runs TimeSheet's ADK-based Agent."""
    ...
    def _run_agent(
        ...
        return self.runner.run_async(
            session_id=session_id, user_id="timesheet_agent", new_message=new_message
        )
    ...
    async def _upsert_session(self, session_id: str):
        session = await self.runner.session_service.get_session(
            app_name=self.runner.app_name, user_id="timesheet_agent", session_id=session_id
        )
        if session is None:
            session = await self.runner.session_service.create_session(
                app_name=self.runner.app_name,
                user_id="timesheet_agent",
